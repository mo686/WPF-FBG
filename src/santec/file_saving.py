...
# FILE_LAST_OPT_PARAMS = "last_opt_params.json"
# FILE_LAST_REF_PARAMS = "last_ref_params.json"
# FILE_LAST_SCAN_REFERENCE_DATA = "last_scan_reference_data.dat"
FILE_LAST_SCAN_PARAMS = os.path.join(CONFIG_DIR, "last_scan_params.json")
FILE_LAST_OPT_PARAMS = os.path.join(CONFIG_DIR, "last_opt_params.json")
FILE_LAST_REF_PARAMS = os.path.join(CONFIG_DIR, "last_ref_params.json")
FILE_LAST_SCAN_REFERENCE_DATA = os.path.join(CONFIG_DIR, "last_scan_reference_data.dat")
...
def save_sts_parameter_data(tsl: TslInstrument,
    ...
    json_data = {
        "selected_chans": [1,2], # ilsts.selected_chans,
...
def check_and_rename_old_file(filename: str):
    ...
    if os.path.exists(filename):
        ...
        str_previous_folder = os.path.join(dir_name, "previous")
        if not os.path.exists(str_previous_folder):
            os.mkdir(str_previous_folder)

        time_now = datetime.now()
        str_new_filename_and_path = os.path.join(
            str_previous_folder, 
            time_now.strftime("%Y%m%d_%H%M%S") + "_" + file_name
        )
...
def move_spectrum_files_to_directory(target_dir, measurement_file, dut_file):
    """将光谱扫描文件移动到指定目录
    
    Args:
        target_dir: 目标目录路径
        measurement_file: 测量数据文件路径
        dut_file: DUT数据文件路径
    """
    import shutil
    ...
    try:
        ...
        if os.path.exists(measurement_file):
            dst_path = os.path.join(target_dir, os.path.basename(measurement_file))
            shutil.move(measurement_file, dst_path)
            print(f"✅ 移动: {os.path.basename(measurement_file)} -> {target_dir}")
            moved = True

        # 移动DUT数据文件
        if os.path.exists(dut_file):
            dst_path = os.path.join(target_dir, os.path.basename(dut_file))
            shutil.move(dut_file, dst_path)
            print(f"✅ 移动: {os.path.basename(dut_file)} -> {target_dir}")
            moved = True
