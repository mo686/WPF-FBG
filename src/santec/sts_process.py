...
class StsProcess(STSData):
    ...
    def set_parameters(self, if_rst_ref=1) -> None:
        """
    ...
    def set_selected_ranges(self, previous_param_data: dict, if_update_range=0) -> None:
        """
        ...
        if if_update_range: 
            self.range = self.selected_ranges
    ...
    def sts_reference(self, prompt_user: bool = True) -> None:
        ...
        for i in self.ref_data:
            if prompt_user:
                ...
                print("\nConnect Slot{} Ch{}".format(i.SlotNumber, i.ChannelNumber))
    ...
    def sts_reference_from_saved_file(self) -> None:
        ...
        # Check if there are any matching channels
        matched_refs = []
        for cached_ref_object in self.reference_data_array:
            for data_st in self.ref_data:
                if (data_st.MPMNumber == cached_ref_object["MPMNumber"] and
                    data_st.SlotNumber == cached_ref_object["SlotNumber"] and
                    data_st.ChannelNumber == cached_ref_object["ChannelNumber"]):
                    matched_refs.append((cached_ref_object, data_st))
                    break

        if not matched_refs:
            logger.error("No matching reference data found for the current selected channels.")
            raise Exception("No matching reference data found for the current selected channels.")

        logger.info(f"Found {len(matched_refs)} matching reference data entries out of {len(self.reference_data_array)} saved entries.")

        for cached_ref_object, matched_data_structure in matched_refs:
        ...
        # Check if all current channels have reference data
        loaded_channels = {(ref[1].SlotNumber, ref[1].ChannelNumber) for ref in matched_refs}
        current_channels = {(data_st.SlotNumber, data_st.ChannelNumber) for data_st in self.ref_data}
        missing_channels = current_channels - loaded_channels
        
        if missing_channels:
            logger.warning(f"Missing reference data for channels: {missing_channels}")
            print(f"Warning: Missing reference data for channels: {missing_channels}")
    ...
    #### 自定义函数 ####
    def read_wavelength_power(self, wavelength, channel_numbers: list = [1]) -> list:
        """
        功能：在指定波长下实时读取多个通道的光功率值
        参数:
            wavelength: 目标波长（单位nm）
            channel_numbers: 通道号列表，例如 [1, 2, 3] 
        返回:
            list: 对应通道的实时光功率值列表（单位：dBm）
        异常:
            Exception: 读取功率失败时抛出
        """      
        if channel_numbers is None:
            # 如果没有指定通道，使用所有选中的通道
            if hasattr(self, 'selected_chans') and self.selected_chans:
                channel_numbers = [int(ch[1]) for ch in self.selected_chans]
            else:
                channel_numbers = [1]  # 默认读取第一个通道      
        try:
            # 设置TSL到指定波长
            self._tsl.set_wavelength(wavelength)
            self._mpm.set_wavelength(wavelength)
            
            # 等待波长稳定
            time.sleep(0.05)
            powers = self._mpm.read_powers()
            # 只返回指定通道的功率值
            selected_powers = [powers[ch_num - 1] for ch_num in channel_numbers if 1 <= ch_num <= len(powers)]
            return selected_powers
        except Exception as e:
            logger.error("读取 %s nm 光功率时出错: %s", wavelength, e)
            return [-110]*len(channel_numbers)  # 返回极低功率值表示读取失败
        
    def resample_reference_data(self) -> None:
        """
        通过插值从现有参考数据生成新的参考数据
        
        参数:
            new_start_wl: 新的起始波长 (nm)
            new_stop_wl: 新的结束波长 (nm)
            new_step: 新的扫描步长 (nm)
        """
        logger.info("Resampling reference data to new wavelength range")
        
        if not self.reference_data_array:
            logger.error("No reference data available for resampling")
            raise Exception("No reference data available for resampling")
        
        # 从当前TSL设置获取波长参数
        new_start_wl = self._tsl.start_wavelength
        new_stop_wl = self._tsl.stop_wavelength
        new_step = self._tsl.sweep_step
        
        resampled_ref_data = []
        
        for ref_item in self.reference_data_array:
            # 获取原始波长和功率数据
            old_wavelengths = np.array(ref_item["rescaled_wavelength"])
            old_powers = np.array(ref_item["rescaled_reference_power"])
            old_monitors = np.array(ref_item["rescaled_monitor"])
            
            # 创建新的波长表
            num_points = int(np.round((new_stop_wl - new_start_wl) / new_step)) + 1
            new_wavelengths = np.linspace(new_start_wl, new_stop_wl, num_points)
            
            # 使用线性插值重采样功率数据
            if len(old_wavelengths) > 1:
                power_interp = interpolate.interp1d(old_wavelengths, old_powers, 
                                                kind='linear', 
                                                bounds_error=False, 
                                                fill_value="extrapolate")
                monitor_interp = interpolate.interp1d(old_wavelengths, old_monitors,
                                                    kind='linear',
                                                    bounds_error=False,
                                                    fill_value="extrapolate")
                
                new_powers = power_interp(new_wavelengths)
                new_monitors = monitor_interp(new_wavelengths)
            else:
                # 如果只有一个数据点，复制到所有新波长
                new_powers = np.full_like(new_wavelengths, old_powers[0])
                new_monitors = np.full_like(new_wavelengths, old_monitors[0])
            
            # 创建新的参考数据项
            new_ref_item = ref_item.copy()
            new_ref_item["rescaled_wavelength"] = new_wavelengths.tolist()
            new_ref_item["rescaled_reference_power"] = new_powers.tolist()
            new_ref_item["rescaled_monitor"] = new_monitors.tolist()
            
            resampled_ref_data.append(new_ref_item)
        
        # 更新参考数据数组
        self.reference_data_array = resampled_ref_data
        logger.info(f"Reference data resampled to {new_start_wl}-{new_stop_wl} nm with {new_step} nm step")

    def sts_reference_from_resampled_data(self) -> None:
        """
        直接从内存中的重采样参考数据加载到STS引擎
        
        Raises:
            Exception: 如果参考数据数组为空
            STSProcessError: 如果加载参考数据失败
        """
        logger.info("Loading STS reference from resampled data in memory")
        
        if self.reference_data_array is None or len(self.reference_data_array) == 0:
            logger.error("The reference data array cannot be null or empty")
            raise Exception("The reference data array cannot be null or empty")

        if len(self.reference_data_array) != len(self.ref_data):
            logger.error("Length mismatch between reference data array and ref data structures")
            raise Exception("Length mismatch between reference data array and ref data structures")
        ...
        for cached_ref_object in self.reference_data_array:
            ...
            print('Loading resampled reference data for Slot{} Ch{}...'.format(
                matched_data_structure.SlotNumber, matched_data_structure.ChannelNumber))

            # 添加MPM通道数据（使用重采样后的数据）
            logger.info("Adding resampled ref mpm channel data")
            errorcode = self._ilsts.Add_Ref_MPMData_CH(cached_ref_object["log_data"], matched_data_structure)
            if errorcode != 0:
                logger.error("Error while loading resampled ref data: %s", 
                            str(errorcode) + ": " + sts_process_error_strings(errorcode))
                raise STSProcessError(str(errorcode) + ": " + sts_process_error_strings(errorcode))

            # 添加监控器数据（使用重采样后的数据）
            logger.info("Adding resampled ref monitor data")
            errorcode = self._ilsts.Add_Ref_MonitorData(
                cached_ref_object["trigger"], 
                cached_ref_object["monitor"],
                matched_data_structure
            )
            if errorcode != 0:
                logger.error("Error while loading resampled ref data: %s",
