...
class GetAddress:
    ...
    def initialize_instrument_addresses(self, 
                                        connection_configuration: str = "GPIB", 
                                        mode: str = "SME", 
                                        tsl_mpm: bool = True, 
                                        daq: bool = True,
                                        auto_select: bool = True) -> None:
        """
        ...
        if tsl_mpm:
            ...
            if auto_select:
                self.auto_select_instruments(instruments)   # 自动选择模式
            else:
                self.select_instruments(instruments)        # 原有交互模式

        if mode == 'SME' and daq:
            if auto_select:
                self.auto_select_daq_device(instruments)
            else:
    ...
    ### 自定义方法
    def auto_select_instruments(self, instruments: list) -> None:
        """自动选择TSL、MPM仪器，减少交互"""
        tsl_found = []
        mpm_found = []
        
        for instr in instruments:
            if 'TSL' in instr.ProductName.upper():
                tsl_found.append(instr)
            elif 'MPM' in instr.ProductName.upper():
                mpm_found.append(instr)
        
        # 自动选择第一个匹配的设备
        if tsl_found:
            self.__cached_tsl_address = tsl_found[0]
            logger.info(f"自动选择TSL: {tsl_found[0].ResourceValue}")
        else:
            raise RuntimeError("未找到TSL激光器")
        
        if mpm_found:
            self.__cached_mpm_address = mpm_found[0] 
            logger.info(f"自动选择MPM: {mpm_found[0].ResourceValue}")
        else:
            raise RuntimeError("未找到MPM功率计")
    
    def auto_select_daq_device(self, instruments: list) -> None:
        """自动选择DAQ设备，避免交互"""
        daq_devices = self._system.devices.device_names
        
        if not daq_devices:
            raise RuntimeError("未找到DAQ设备")
        
        # 如果有唯一设备则自动选择
        if len(daq_devices) == 1:
            self.__cached_daq_address = daq_devices[0]
            logger.info(f"自动选择DAQ设备: {daq_devices[0]}")
        else:
            # 多个设备时选择名称包含"PXI"的设备
            pxi_devices = [dev for dev in daq_devices if 'PXI' in dev.upper()]
            if pxi_devices:
                self.__cached_daq_address = pxi_devices[0]
                logger.info(f"自动选择PXI设备: {pxi_devices[0]}")
            else:
                # 默认选择第一个设备
                self.__cached_daq_address = daq_devices[0]
                logger.info(f"自动选择第一个DAQ设备: {daq_devices[0]}")