...
class TslInstrument(TslData):
    ...
    def wait_for_sweep_status(self,
        ...
        if errorcode != 0:
            error_msg = str(errorcode) + ": " + instrument_error_strings(errorcode)
            logger.error("Error while TSL wait for sweep status: %s", error_msg)
            raise InstrumentError(error_msg)
