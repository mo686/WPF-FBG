...
class MpmInstrument(MpmData):
    ...
    def set_wavelength(self, wavelength):
        """设置测量波长 (手册5.5.3节，命令10)"""
        try:
            command = f"WAV {wavelength}"
            status = self.write(command)
            # if status == 0:
            #     print("波长设置成功")
            # else:
            #     print(f"波长设置失败，状态码: {status}")
        except Exception as e:
            print(f"设置波长时出错: {e}")
    ...
    def read_powers(self):
        ...
        参数:
            module: 模块编号 (0-4)
        返回:
            包含4个端口功率值的列表 [port1, port2, port3, port4]
        """
        try:
            status, response = self.query("READ? 0")
            if status == 0:
                # 响应格式: value1,value2,value3,value4
                # 例如: "-20.123,-20.454,-20.764,-20.644"
                power_values = [float(x) for x in response.split(',')]
                # print(f"CH1: {power_values[0]} dBm")
                # print(f"CH2: {power_values[1]} dBm") 
                # print(f"CH3: {power_values[2]} dBm")
                # print(f"CH4: {power_values[3]} dBm")
                return power_values
            else:
                print(f"读取光功率失败，状态码: {status}")
        except Exception as e:
            print(f"读取光功率时出错: {e}")
    ...
    def cls_status(self): 
        ...
        # 3. 清空错误队列
        error_count = 0
        while error_count < 20:  # 防止无限循环
            status, response = self.query("ERR?")
            if status == 0:
                break